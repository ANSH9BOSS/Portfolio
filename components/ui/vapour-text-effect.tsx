  ...
  particles.forEach(particle => {
    ...
    if (shouldVaporize) {
      ...
      if (particle.shouldFadeQuickly) {
        ...
        particle.velocityX = (particle.velocityX + (Math.random() - 0.5) * 1.5 + dx * 0.002) * damping;
        particle.velocityY = (particle.velocityY + (Math.random() - 0.5) * 1.5 + dy * 0.002) * damping;
        particle.x += particle.velocityX * deltaTime * 20;
        particle.y += particle.velocityY * deltaTime * 10;
        particle.opacity = Math.max(0, particle.opacity - deltaTime * 0.2);
      }
...
function transformValue(input: number, inputRange: number[], outputRange: number[], clamp = false): number {
  const [iM, iX] = inputRange; const [oM, oX] = outputRange;
  const progress = (input - iM) / (iX - iM);
  let res = oM + progress * (oX - oM);
  if (clamp) res = oX > oM ? Math.min(Math.max(res, oM), oX) : Math.min(Math.max(res, oX), oM);
  return res;
}
